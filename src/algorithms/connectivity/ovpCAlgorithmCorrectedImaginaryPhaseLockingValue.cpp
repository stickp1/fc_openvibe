...
#include "ovpCAlgorithmCorrectedImaginaryPhaseLockingValue.h"
#include <cmath>
#include <complex>
#include <Eigen/Dense>
...
boolean CAlgorithmCorrectedImaginaryPhaseLockingValue::initialize(void)
{
...
boolean CAlgorithmCorrectedImaginaryPhaseLockingValue::uninitialize(void) {
...
boolean CAlgorithmCorrectedImaginaryPhaseLockingValue::process(void)
{
	...
	if(this->isInputTriggerActive(OVP_Algorithm_Connectivity_InputTriggerId_Initialize))
	{
		...
		{
			this->getLogManager() << LogLevel_Error << "Can't compute ciPLV on two signals with different lengths";
			return false;
		}
		...
		if(l_ui32SamplesPerChannel1<2||l_ui32SamplesPerChannel2<2)
		{
			...
			this->getLogManager() << LogLevel_Error << "Can't compute ciPLV, input signal size = "<<l_size<<"\n";
			return false;
		}
		...
		for(uint32 channel = 0; channel < l_ui32PairsCount; channel++)
		{
			...
			if(m_pHilbertTransform->process(OVP_Algorithm_HilbertTransform_InputTriggerId_Initialize)) // Check initialization before doing the process on channel 1
			{
				...
				if(m_pHilbertTransform->process(OVP_Algorithm_HilbertTransform_InputTriggerId_Initialize)) // Check initialization before doing the process on channel 2
				{
					...
					// Compute ciPLV and store it in l_opMatrixBuffer for each pair
					...
					std::complex<double> num = imag(l_sum)/l_ui32SamplesPerChannel1;
					std::complex<double> den = sqrt(1-pow(real(l_sum)/l_ui32SamplesPerChannel1,2));
					l_opMatrixBuffer[channel] = abs(num/den);
					
					if(l_opMatrixBuffer[channel]!=l_opMatrixBuffer[channel]) // check for 0/0 indeterminacy 
						l_opMatrixBuffer[channel]=0;
					//cout <<"Corrected Imaginary PLV: "<< l_opMatrixBuffer[channel] << endl;
				}
