...
#include "ovpCAlgorithmImaginaryCoherencyHilbert.h"
#include <cmath>
#include <complex>
#include <Eigen/Dense>
...
boolean CAlgorithmImaginaryCoherencyHilbert::initialize(void)
{
...
boolean CAlgorithmImaginaryCoherencyHilbert::uninitialize(void) {
...
boolean CAlgorithmImaginaryCoherencyHilbert::process(void)
{
	...
	if(this->isInputTriggerActive(OVP_Algorithm_Connectivity_InputTriggerId_Process))
	{
		...
		for(uint32 channel = 0; channel < l_ui32PairsCount; channel++)
		{
			...
			if(m_pHilbertTransform->process(OVP_Algorithm_HilbertTransform_InputTriggerId_Initialize)) // Check initialization before doing the process on channel 1
			{
				...
				{
					l_vecXdAnalytic1(i) = op_pHilbertMatrix->getBuffer()[i]; // Store instantaneous phase given by Hilbert algorithm
				...
				if(m_pHilbertTransform->process(OVP_Algorithm_HilbertTransform_InputTriggerId_Initialize)) // Check initialization before doing the process on channel 2
				{
					...
					for(uint32 i=0; i<l_ui32SamplesPerChannel1; i++)
					{
						...
						//cout<<"Sinal analítico 1 sample nr "<<i<<": "<<l_vecXdChannelToCompare1(i)+iComplex*l_vecXdAnalytic1(i)<<endl;
						//cout<<"Sinal analítico 2 sample nr "<<i<<": "<<l_vecXdChannelToCompare2(i)-iComplex*l_vecXdAnalytic2(i)<<endl;
						num += (l_vecXdChannelToCompare1(i)+iComplex*l_vecXdAnalytic1(i))*(l_vecXdChannelToCompare2(i)-iComplex*l_vecXdAnalytic2(i));
						//num += imag(l_vecXdAnalytic1(i)*conj(l_vecXdAnalytic2(i)));	
						den1 += (l_vecXdChannelToCompare1(i)+iComplex*l_vecXdAnalytic1(i))*(l_vecXdChannelToCompare1(i)-iComplex*l_vecXdAnalytic1(i));;
						den2 += (l_vecXdChannelToCompare2(i)+iComplex*l_vecXdAnalytic2(i))*(l_vecXdChannelToCompare2(i)-iComplex*l_vecXdAnalytic2(i));
					}


					//l_opMatrixBuffer[channel] = abs(l_sum)/l_ui32SamplesPerChannel1;
					//cout<<"Numerador:sum: imag(hilbert*conj(hilbert)) "<<num<<endl;
					//cout<<"Denominador1:sum: hilbert1^2 "<<den1<<endl;
					//cout<<"Denominador2:sum: hilbert2^2 "<<den2<<endl;
					l_opMatrixBuffer[channel] = abs(imag(num)/sqrt(den1*den2));
					//cout <<"Imaginary Hilbert Coherency: "<< l_opMatrixBuffer[channel] << endl;
				}
